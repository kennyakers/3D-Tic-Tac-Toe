/*
// To submit if other version doesn't work.

import java.util.ArrayList;

public class Board {

    private final double OPPORTUNITY_THREE_FACTOR = 10000.0;
    private final double OPPORTUNITY_TWO_FACTOR = 1000.0;
    private final double OPPORTUNITY_ONE_FACTOR = 10.0;

    public int[][][] board;
    private static ArrayList<Goal> goalStates;

    public int DEBUG_totalFilled = 0;
    ...
    public int evaluationFunction(int playerID) {
        ...
        for (Goal goalState : goalStates) {
            ...
            switch (Math.abs(counter)) {
                case 4:
                    if (counter > 0) {
                        totalFilled = Integer.MAX_VALUE; // Keep this stuff.
                    } else if (counter < 0) {
                        totalFilled = Integer.MIN_VALUE;
                    }
                    break;
                case 3:
                    // 3 in this row
                    totalFilled += (Math.signum(counter) * OPPORTUNITY_THREE_FACTOR);
                    break;
                case 2:
                    // 2 in this row
                    totalFilled += (Math.signum(counter) * OPPORTUNITY_TWO_FACTOR);
                    break;
                case 1:
                    // 1 in this row
                    totalFilled += (Math.signum(counter) * OPPORTUNITY_ONE_FACTOR);
                    break;
            }
        }

        this.DEBUG_totalFilled = totalFilled;
...
// Version with most complex eval funciton (not sure if working).
import java.util.ArrayList;
...
public class Board {
    ...
    private final double POWER_POSITION_FACTOR = 3.0;
    private final double POWER_OPPONENT_POSITION_FACTOR = 1.5;

    private final double BLOCKING_THREE_FACTOR = this.OPPORTUNITY_THREE_FACTOR - 10.0; // -10 is arbitrary, used for weight-clashing experiment
    private final double BLOCKING_TWO_FACTOR = this.OPPORTUNITY_TWO_FACTOR - 10.0;
    private final double BLOCKING_ONE_FACTOR = this.OPPORTUNITY_ONE_FACTOR - 10.0;
    ...
    public int blockingFactor(int currentPlayer, int opponent) {
        ...
        for (Goal goalState : goalStates) {
            ...
            switch (numOpponentPiecesInGoalState) {
                case 1:
                    count += (Game.PRIORITIZE_CORNER_MOVES && goalState.hasCorners() ? this.CORNER_FACTOR : 1) * (Game.PRIORITIZE_MULTILEVEL_MOVES && goalState.isMultiLevel() ? this.MULTILEVEL_FACTOR : 1) * (numOpponentPiecesInGoalState * this.BLOCKING_ONE_FACTOR);
                    break;
                case 2:
                    count += (Game.PRIORITIZE_CORNER_MOVES && goalState.hasCorners() ? this.CORNER_FACTOR : 1) * (Game.PRIORITIZE_MULTILEVEL_MOVES && goalState.isMultiLevel() ? this.MULTILEVEL_FACTOR : 1) * (numOpponentPiecesInGoalState * this.BLOCKING_TWO_FACTOR);
                    break;
                case 3:
                    count += (Game.PRIORITIZE_CORNER_MOVES && goalState.hasCorners() ? this.CORNER_FACTOR : 1) * (Game.PRIORITIZE_MULTILEVEL_MOVES && goalState.isMultiLevel() ? this.MULTILEVEL_FACTOR : 1) * (numOpponentPiecesInGoalState * this.BLOCKING_THREE_FACTOR);
                    break;
            }
        }
        return count;
    ...
    public int evaluationFunction(int playerID) {
        ...
        if (Game.CONSIDER_POWER_POSITIONS) {
            for (int i = 0; i < board.length; i++) {
                for (int j = 0; j < board[i].length; j++) {
                    for (int k = 0; k < board[i][j].length; k++) {
        ...
        for (Goal goalState : goalStates) {
            ...
            switch (Math.abs(counter)) {
                ...
                case 3:
                    ...
                    totalFilled += (Game.PRIORITIZE_CORNER_MOVES && goalState.hasCorners() ? (Game.COUNT_TURNS ? (this.CORNER_FACTOR * this.turnCount) : this.CORNER_FACTOR) : 1) * (Game.PRIORITIZE_MULTILEVEL_MOVES && goalState.isMultiLevel() ? (Game.COUNT_TURNS ? (this.turnCount * this.MULTILEVEL_FACTOR) : this.MULTILEVEL_FACTOR) : 1) * ((Math.signum(counter) * OPPORTUNITY_THREE_FACTOR));
                    break;
                case 2:
        ...
        if (numMyThreeInARows >= 2) {
            totalFilled += this.OPPORTUNITY_THREE_FACTOR;
        ...
        return (int) ((this.POWER_POSITION_FACTOR * numMyPowerPositions) - (this.POWER_OPPONENT_POSITION_FACTOR * numOpponentPowerPositions)) + totalFilled + (Game.CONSIDER_BLOCKING ? this.blockingFactor(playerID, opponentID) : 0);
